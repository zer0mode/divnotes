...
To simplify it add this line in file .bashrc as alias
...
Finaly I place projects in ~/dev folder. For more convenience, below is the proposed djangoproject.com solution.
...
##### SQLite installalation
If error appears ot there is sqlite is not on system, build it from source :
...
### Spatial libraries
For managing spatial data SpatiaLite component is required.
...
#### PROJ.4
Geospatial data converter for differen coordinate reference systems
...
><sub>dal-$VERSION.tar.gz=2.1.1</sub>
...
Optional libraries :
<sup>http://postgis.net/docs/manual-2.1/postgis\_installation.html#install\_requirements</sup>
...
It might come handy to extract data out of xls files, it's what FreeXL is for. If so, install first the dependancy <sup>https://www.gaia-gis.it/fossil/freexl/index</sup> following the usual `>BUILD` procedure and then build SpatiaLite with full dependency support.

	>BUILD

Spatialite variable has to be set in setting.py for versions 4.2+
...
A friendly *meta-variable* `>BUILD` which contains commands for making and installing is used in this file to make it shorter and more comprehensible. Whenever you see it, its means the following block :
...
https://pypi.python.org/packages/cc/a4/023ee9dba54b3cf0c5a4d0fb2f1ad80332ef23549dd4b551a9f2cbe88786/pysqlite-2.8.2.tar.gz#md5=b8488f6a353bd6a3cd85d327d326376a
...
This guide is inspired essentialy from [djangoproject](https://docs.djangoproject.com/en/1.9/ref/contrib/gis/install/).

References are included for particular installation steps & details.
...
# pgrouting"
	# If not installed at this point it can be done later, depending on the type on 
...
http://gis.stackexchange.com/questions/192650/postgis-broken-after-ubuntu-16-04-upgrade/#202729
basic install script example ( to adapt ) : http://gis.stackexchange.com/questions/167687/how-to-enable-sfcgal-in-postgis#188649

Check psql_how_to for setting up a postgresql spatial database in django
